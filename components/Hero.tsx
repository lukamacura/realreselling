...
// Typewriter hook — brz, UX-friendly i gasi se pri prefers-reduced-motion
function useTypewriter({
  text,
  speed = 18,
  startDelay = 120,
  enabled = true,
}: { text: string; speed?: number; startDelay?: number; enabled?: boolean }) {
  const [out, setOut] = useState(enabled ? "" : text);
  const [done, setDone] = useState(!enabled);
  useEffect(() => {
    if (!enabled) { setOut(text); setDone(true); return; }
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    let i = 0; let cancelled = false; let timeout: any; const kick = setTimeout(function tick(){
      if (cancelled) return; i += 1; setOut(text.slice(0, i)); if (i < text.length) { timeout = setTimeout(tick, speed); } else { setDone(true); }
    }, startDelay);
    return () => { cancelled = true; clearTimeout(kick); clearTimeout(timeout); };
...
export default function Hero() {
  ...
  const { out: typedOut, done } = useTypewriter({ text: typedHeadline, speed: 18, startDelay: 120, enabled: !prefersReduce });

  return (
    <section className="relative mx-auto mt-10 overflow-hidden bg-brand-dark py-10 sm:py-14 md:mt-40 md:py-0">
      {/* Zlatni radijal desno (blago "diše") */}
      <motion.div
        aria-hidden
          ...
          >
            <span className="m-0 block font-display text-[40px] leading-tight text-brand-gold md:text-[64px]">
              ...
              <span className="ml-2 text-white" aria-label={typedHeadline} aria-live="polite">
                {typedOut}
                {!done && (
                  <motion.span aria-hidden className="inline-block w-[0.5ch] translate-y-[0.05em]">
                    |
                  </motion.span>
                )}
              </span>
      ...
      {/* Prefer-reduced-motion podrška (Tailwind rješenje kroz utility klase nije potrebno; Framer radi automatski) */}
    </section>
  );
}
